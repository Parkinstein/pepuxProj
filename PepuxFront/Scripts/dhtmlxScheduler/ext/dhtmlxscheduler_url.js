...
if(a){a=!1;var s=e._get_url_nav();if(s.event)try{if(e.getEvent(s.event))return t(s.event),!1;var c=e.attachEvent("onXLE",function(){t(s.event),e.detachEvent(c)})}catch(u){}if(s.date||s.mode){try{this.setCurrentView(s.date?n(s.date):null,s.mode||null)}catch(u){this.setCurrentView(s.date?n(s.date):null,d)}return!1}}var p=["date="+i(_||l),"mode="+(d||o)];r&&p.push("event="+r);var h="#"+p.join(",");return document.location.hash=h,!0})})});